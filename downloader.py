...
class LikeeDownloader:
    ...
    def capture_screenshot(self):
        ...
        print(f"[~] Captured: downloads/screenshots/{args.username}_likee-scraper.png")
    ...
    def download_user_videos(self):
        ...
        print("[~] Getting videos...", end='')
        response = requests.post(self.user_videos_api_endpoint, json=payload).json()
        ...
        for video in videos[:10]:
            ...
            with open(f"downloads/videos/{args.username}_{video['postId']}.mp4", "wb") as file:
                for chunk in tqdm(response.iter_content(chunk_size=1024 * 1024),
                                  desc=f"Downloading: {video['postId']}.mp4"):
                    if chunk:
                        file.write(chunk)

            print(f"Downloaded: {file.name}\n")
        print(f"[+] Complete: {downloaded_videos} videos were downloaded.")
...
parser.add_argument('--videos', help='download videos (downloads 10 videos maximum)', action='store_true')
parser.add_argument('--screenshot', help='capture screenshot of target\'s profile', action='store_true')
parser.add_argument('-d', '--debug', help='enable debug mode', action='store_true')
parser.add_argument('-u', '--update', help='update program', action='store_true')
parser.add_argument('-v', '--version', version='1.0', action='version')
...
if __name__ == "__main__":
    try:
        if args.update:
            LikeeDownloader(args).get_updates()
        else:
        ...
        raise Exception(f"[x] An error occured: {e}")
